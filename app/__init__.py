...
def create_app(config=Config):
    ...
    from app.core import bp as core_bp              # blueprint from core __init__.py
    flask_app.register_blueprint(core_bp, url_prefix='')
    # connect the authentication endpoints under the prefix /auth
    from app.auth import bp as auth_bp      #core blueprint has the routes on it, no prefix on them? there's also auth routes
    flask_app.register_blueprint(auth_bp, url_prefix='/auth')
    
    # register the database models
    db.init_app(flask_app)          #create sqlalchemy database
